...
HTTP 2는 이같은 스트림에서의 흐름제어를 직접적으로 구현하지 않고, 어플리케이션 계층의 응용프로그램 개발자가 이를 구현할 수 있도록 간단한 빌딩 블록을 제공한다. 응용프로그래머는 요청 처리에서의 흐름제어를 자신들만의 목적과 그에 대응하는 알고리즘으로 구현할 수 있고, 또는 완전히 구현하지 않을 수도 있다.
