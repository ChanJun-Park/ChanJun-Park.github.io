...
- 전체 시스템의 구성요소로 훨씬 쉽게 파악할 수 있게 해준다.
...
**인터넷 프로토콜 5계층** 에 추가적으로 2개의 계층을 추가해서 인터넷 계층을 표현한 것이 **OSI 7계층** 이다. **프레젠테이션 계층**과 **세션 계층** 이 추가되었으며, 데이터의 보안과 표현 등을 처리하는 계층이다. 이들 계층은 어플리케이션 계층에서 추가적으로 구현될 수 있으며 이 계층을 구현할지 말지는 프로그래머의 선택해 달려있다.
